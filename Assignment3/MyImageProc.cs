...
namespace ImageProcesing2010
{
    ...
    public class MyImageProc
    {
        ...
        //assuming input data as the output data of the nonMaxSupression function
        public static Boolean doubleThresholding(Bitmap Data, int lowThreshhold, int highThreshhold)
        {
            int KernelSize = 5;
            int Limit = KernelSize / 2;
            
            for (int i = Limit; i <= (Data.Width - Limit) - 1; i++)
            {
                for (int j = Limit; j <= (Data.Height - Limit) - 1; j++)
                {
                    //considering R=G=B
                    if (Data.GetPixel(i, j).R < lowThreshhold)
                    {
        ...
        public static Boolean NonMaximumSupression(Bitmap Data)
        {
            // Perform Non maximum suppression:
            // NonMax = Gradient;
            Bitmap derivativeX = Data;
            Bitmap derivativeY = Data;
            double[,] Gradient = new double[Data.Width, Data.Height];
            int KernelSize = 5;
            ...
            //filter image using both sobel operator to obtain gradients
            ...
            Convolve(derivativeY, kernelY);

            //Evaluate Gradient matrix
            ...
            for (int i = Limit; i <= (Data.Width - Limit) - 1; i++)
            {
                ...
                {
                    //considering R=G=B
                    double G= Math.Sqrt( (derivativeX.GetPixel(i, j).R * derivativeX.GetPixel(i, j).R) 
                        + (derivativeY.GetPixel(i, j).R * derivativeY.GetPixel(i, j).R) );
                   Gradient[i, j] = G;
                }
            }

            //Convolve(Gradient, kernelX);
            //Convolve(Gradient, kernelY);
            ...
            for (int i = Limit; i <= (Data.Width - Limit) - 1; i++)
            {
                for (int j = Limit; j <= (Data.Height - Limit) - 1; j++)
                {
                    ...
                    if (derivativeX.GetPixel(i, j).R == 0)
                        Tangent = 90F;
                    else
                        Tangent = (float)(Math.Atan(derivativeX.GetPixel(i, j).R / derivativeX.GetPixel(i, j).R) * 180 / Math.PI); //rad to degree
        ...
        public static Boolean Differentiate(Bitmap Data, int[,] Filter)
        {
            int i, j, k, l, Fh, Fw;
            ...
            for (i = Fw / 2; i <= (Data.Width - Fw / 2) - 1; i++)
            {
                for (j = Fh / 2; j <= (Data.Height - Fh / 2) - 1; j++)
                {
                    sumR = 0;
                    sumG = 0;
                    sumB = 0;
                    for (k = -Fw / 2; k <= Fw / 2; k++)
                    {
                        for (l = -Fh / 2; l <= Fh / 2; l++)
                        {
                            sumR = sumR + Data.GetPixel(i + k, j + l).R * Filter[Fw / 2 + k, Fh / 2 + l];
                            sumG = sumG + Data.GetPixel(i + k, j + l).G * Filter[Fw / 2 + k, Fh / 2 + l];
                            sumB = sumB + Data.GetPixel(i + k, j + l).B * Filter[Fw / 2 + k, Fh / 2 + l];
                        }
                    }
                    //Output[i, j] = sum;
